    ...
    for r, d, f in os.walk(path):
        for file in f:
            if '.csv' in file:
                ...
                # for each new track create three "zeros" columns
                # one for x, y, and track number
                # zeros are handled as nonexistent instances
                tracks = np.append(tracks, np.zeros([numFrames + 1, 3]), axis=1)
                ...
                with open(files[imported]) as csv_file:
                    ...
                    for row in csv_reader:
                        # tracks.insert())
                        tracks[int(row[0]) - 1, imported * 3 + 1] = int(row[1])
                        tracks[int(row[0]) - 1, imported * 3 + 2] = int(row[2])
                        tracks[int(row[0]) -1, imported * 3 + 3] = int(file.split('.')[1]) # add track number
                        line_count += 1
                    
                    print("imported", str(file), f' with {line_count} points.')
                    ant_num.append(int(file.split('.')[1]))
                imported += 1
...
def display_video(cap, tracks, show=(0, math.inf), scale=1.0):
    ...
    while True:  # run until no more frames are available
        ...
        for track in range(math.floor(((tracks.shape[1]) - 1) / 3)):
            if tracks[frame_num, track * 3 + 1] != 0:
                # the tracks are read as centres
                target_centre = np.asarray([tracks[frame_num, track * 3 + 1], tracks[frame_num, track * 3 + 2]])
                ...
                cv2.putText(frame, "track: " + str(tracks[frame_num, track * 3 + 3]),
...
def save_frame(frame_num, tracks, cap, export, scale=1):
    """
    adds labels to frame and saves it

    :param frame_num: desired frame 
    :param tracks: array of all labelled tracks imported
    """   

    tracks = (scale * tracks).astype(int)  
...
def extractPatches(frame_no, frames, tracks, patch_size=128, BW=True):
    ...
    for track in range(1, tracks.shape[1], 3):
        stack = []
        pos = []
        no_detection = 0
        ...
        if no_detection != len(frames):
            ...
            label = tracks[frame_no + (img - len(frames) + num_empty_img), track+2]
...
def save_patches(stacks, stack_labels, folder):
    for i in range(len(stacks)):
        name = "%s/label_%s.jpg".format(folder, str(stack_labels[i]), stacks[i][0])
        cv2.imwrite(name, i)
